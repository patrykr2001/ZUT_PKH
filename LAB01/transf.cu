...
int main(void) {
	...
	for(int i = 1000; i <= 10000000; i *= 10){
		...
		for(int j = 1; j <= 5; j++){
			...
			cudaMalloc(&d_W, N * sizeof(WIERZCHOLEK));
			cudaMemcpy(d_W, Figura2, N * sizeof(WIERZCHOLEK), cudaMemcpyHostToDevice);
			int watki_na_blok = 1024;
			int bloki_na_siatke = (N + watki_na_blok - 1) / watki_na_blok;
			...
			// for(int i = 0; i < N; i++){
			// 	if(Figura1[i].x != Figura2[i].x || Figura1[i].y != Figura2[i].y){
			// 		printf("Błędne dane:\n");
			// 		printf("CPU:");
			// 		printf(" %f,%f", Figura1[i].x, Figura1[i].y);
			// 		printf("\tGPU:");
			// 		printf(" %f,%f\n", Figura2[i].x, Figura2[i].y);
			// 	}
			// }
